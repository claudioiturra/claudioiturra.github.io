...
(function( root, factory ) {
			...
			if( !config.disableLayout ) {
				...
				if( scale === 1 ) {
					...
					if( scale > 1 && features.zoom && window.devicePixelRatio < 2 ) {
						dom.background.style.zoom =  dom.slides.style.zoom = scale;
						dom.background.style.left = dom.slides.style.left = '';
						dom.background.style.top = dom.slides.style.top = '';
						dom.background.style.bottom =  dom.slides.style.bottom = '';
						dom.background.style.right =  dom.slides.style.right = '';
						transformSlides( { layout: '' } );
					...
					// high scale values on ldpi screens.
						...
						dom.background.style.zoom =  dom.slides.style.zoom = '';
						dom.background.style.left = dom.slides.style.left = '50%';
						dom.background.style.top = dom.slides.style.top = '50%';
						dom.background.style.bottom =  dom.slides.style.bottom = 'auto';
						dom.background.style.right =  dom.slides.style.right = 'auto';
						transformSlides( { layout: 'translate(-50%, -50%) scale('+ scale +')' } );
						dom.background.style.transform = 'translate(-50%, -50%) scale(' + scale + ')';
					}
				}
				/* Added for Slideas */
    ...
    // replace the content of the div by the youtube iFrame
    function insertYouTubeIFrame(videoWrapper) {
        // prepare youtube iFrame
        var iframe = document.createElement("iframe");
        var embed = "https://www.youtube.com/embed/ID?autoplay=1";
        var videoId = videoWrapper.getAttribute('data-id');
	...
	Reveal = {
        ...
        insertYouTubeIFrame: insertYouTubeIFrame,
